...
export class StudentResultComponent implements OnInit {
  ...
  getStudentResults(user_id: any){
    this._resultSvc.getStudentResultByUser(user_id).subscribe( 
      ...
      (error) => {
        this.messageService.add({key: 'success', severity:'warn', summary: 'Notification Message', detail: error?.error?.error });
      });   
  }
  ...
  submitFeedback(){
    ...
    let data = {'student_id': student_id, 'class_id': class_id, 'message': message, 'exam_id': this.student_details.exam_id};
    ...
    this._resultSvc.postParentFeedback(data).subscribe( 
      (response) => {
        ...
        this.feedback = this.res.feedback;
        this.messageService.add({key: 'success', severity:'success', summary: 'Success Message', detail: this.success_message});
      }, 
      (error) => {
        this.messageService.add({key: 'success', severity:'warn', summary: 'Notification Message', detail: error?.error?.error });
      });
